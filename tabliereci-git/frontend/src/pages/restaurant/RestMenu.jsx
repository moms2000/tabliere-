...
import { Notebook, Plus, Pencil, Trash2, QrCode, Eye, EyeOff } from "lucide-react";
  ...
  const toggleItem = (catId, itemId) =>
    setCategories(prev => prev.map(c =>
      c.id === catId ? { ...c, items: c.items.map(i =>
        i.id === itemId ? { ...i, active: !i.active } : i
      )} : c
    ));

  const activeCategory = categories.find(c => c.id === activeTab);
  const totalItems     = categories.reduce((a, c) => a + c.items.length, 0);
  const activeItems    = categories.reduce((a, c) => a + c.items.filter(i => i.active).length, 0);
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: qrActive ? "#1D9E75" : "#aaa", marginTop: 2 }}>
                {qrActive
                  ? "tabliereci.ci/menu/le-maquis-du-plateau · clients scannent pour commander"
                  : "Activez pour permettre aux clients de commander via QR"}
              </div>
            </div>
            <Toggle value={qrActive} onChange={setQrActive} />
          </div>
        </Card>
      </motion.div>
            ...
            {categories.map(c => {
              ...
              return (
                <div key={c.id} onClick={() => setActiveTab(c.id)}
                  ...
                  <span>{c.name}</span>
                  <span style={{ fontSize: 11, color: "#aaa" }}>{c.items.length}</span>
                </div>
              );
            })}
            <div style={{ marginTop: 10, paddingTop: 10, borderTop: "0.5px solid #f0f0f0" }}>
              <Btn variant="default" icon={Plus} style={{ width: "100%", justifyContent: "center", fontSize: 12 }}>
                Catégorie
              </Btn>
            </div>
          </Card>
                ...
                {activeCategory?.items.map((item, i) => (
                  <motion.div key={item.id}
                    ...
                    <div style={{ width: 4, height: 36, borderRadius: 2,
                      background: item.active ? "#1D9E75" : "#ddd", flexShrink: 0 }} />
                    <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                      <Toggle value={item.active} onChange={() => toggleItem(activeCategory.id, item.id)} />
