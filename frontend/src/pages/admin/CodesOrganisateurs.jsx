...
export default function CodesOrganisateurs() {
  ...
  const load = () => {
    ...
    api.get("/admin/organisateur-codes", { params })
      .then(r => {
        setCodes(r.data?.data || []);
        setTotal(r.data?.pagination?.total ?? r.data?.data?.length ?? 0);
      })
      .catch(err => console.error("Erreur chargement codes", err?.response?.data || err.message))
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      const r = await api.post("/admin/organisateur-codes/generate", {
        count: genCount,
  ...
  const deleteCode = async (id) => {
    ...
    try {
      await api.delete(`/admin/organisateur-codes/${id}`);
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show">
      <motion.div variants={fadeUp}>
        <PageTitle title="Codes Organisateurs" subtitle="Générez des codes d'accès pour les organisateurs d'événements" />
      </motion.div>
        ...
        <Card style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "80px 1fr 100px auto", gap: 10, alignItems: "end" }}>
            <div>
              ...
              <input type="number" min={1} max={50} value={genCount} onChange={e => setGenCount(+e.target.value)} style={inp} />
            </div>
            <div>
              <label style={lbl}>Note interne (optionnel)</label>
              <input value={genNotes} onChange={e => setGenNotes(e.target.value)}
                placeholder="Ex: Soirée Nouvel An - promoteur Koffi" style={inp} />
            </div>
            <div>
              <label style={lbl}>Expire (jours)</label>
              <input type="number" min={0} value={genExpiry} onChange={e => setGenExpiry(+e.target.value)} style={inp} />
            </div>
              ...
              {["all","unused","used"].map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  style={{ fontSize: 11, padding: "4px 10px", borderRadius: 8, cursor: "pointer",
                    ...
                    color: filter === f ? "#C47D1A" : "#888", fontWeight: filter === f ? 600 : 400 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontSize: 16, fontWeight: 800, fontFamily: "monospace",
                        color: c.is_used ? "#bbb" : "#1E2E28", letterSpacing: "1px" }}>{c.code}</span>
                      <button onClick={() => copyCode(c.code)} style={{ border: "none", background: "none", cursor: "pointer", padding: 2 }}>
                        {copied === c.code ? <Check size={14} color="#1D9E75" /> : <Copy size={14} color="#bbb" />}
                      </button>
                      <Badge label={c.is_used ? "Utilisé" : "Disponible"} variant={c.is_used ? "gray" : "green"} />
                    </div>
...
const lbl = { display: "block", fontSize: 10, fontWeight: 700, color: "#9BA89F", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 6 };
const inp = { width: "100%", border: "0.5px solid #E4DFD8", borderRadius: 8, padding: "9px 12px", fontSize: 13, outline: "none", background: "#F8F5EF" };
