...
export const eventOpsService = {
  ...
  async listMyOrders(orderToken)                 { return (await api.get("/event-orders/mine", { params: { order_token: orderToken } })).data.data; },
  ...
  async recordBalance(resaId, amount, method, auth, eventId) { return (await api.post(`/event-checkin/${resaId}/balance`, { amount, method, event_id: eventId }, authCfg(auth))).data.data; },
